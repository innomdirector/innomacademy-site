...
const sectionSubtitles = {
  tracks: 'საიტები, აპები და AI მარტივი გზით',
  creative: 'ვიზუალური მიმართულება',
}

const mapAdultSections = (courses) => {
  const grouped = {}

  courses.forEach((course) => {
    const catalog = course.catalog?.adult
    if (!catalog) return

    const sectionId = catalog.sectionId || 'tracks'
...
const AdultCourses = () => {
  const [openIds, setOpenIds] = useState(['tracks'])
  ...
  const sections = useMemo(
    () => mapAdultSections(coursesCatalog.courses || []),
  ...
  return (
    <div id="adult-courses" className="py-12 md:py-16 font-mersad2 mt-10 md:mt-20">
      <WarningToast
          ...
          <div className="space-y-6">
            {sections.map((section) => {
              ...
              return (
                <div
                  key={section.id}
                  className="group relative overflow-hidden rounded-3xl border border-white/10 bg-[#0b111e]/85 backdrop-blur-xl shadow-[0_22px_70px_-40px_rgba(15,23,42,0.95)]"
                >
                  <button
                    type="button"
                    onClick={() => toggleSection(section.id)}
                    className="w-full text-left px-6 py-6 flex items-center justify-between gap-6 bg-[linear-gradient(180deg,rgba(15,23,42,0.9)_0%,rgba(11,17,30,0.92)_100%)] hover:bg-[#0f172a] transition-colors"
                  >
                    <div>
                      ...
                      <div className="mt-3 h-0.5 w-14 rounded-full bg-[#22d3ee]/70" />
                    </div>
                    <span
                  ...
                  <AnimatePresence initial={false}>
                    {isOpen && (
                      ...
                      >
                        <div className="px-6 py-6 bg-[#0a1326]/90 border-t border-white/10">
                          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            {section.courses.map((course) => {
                              const cardClass =
                                'rounded-2xl border border-white/10 bg-[#0c1324]/90 p-5 shadow-[0_14px_34px_-22px_rgba(15,23,42,0.95)] transition hover:-translate-y-1 hover:border-[#22d3ee]/70'
                                  ...
                                  <motion.article key={course.id} whileHover={{ y: -4 }} className={cardClass}>
                                    ...
                                    >
                                      <div className="flex items-start flex-col md:flex-row md:items-center gap-3">
                                        <div className="shrink-0 self-start w-12 h-12 rounded-xl bg-[linear-gradient(135deg,#22d3ee_0%,#38bdf8_100%)] flex items-center justify-center text-2xl ring-1 ring-white/15">
                                          {course.icon}
                                        </div>
                                        <div>
                                          <div className="text-lg font-semibold text-white">{course.title}</div>
                                          <div className="text-sm text-white/65">{course.description}</div>
                                        </div>
                                      </div>
                                    </Link>
                                  </motion.article>
